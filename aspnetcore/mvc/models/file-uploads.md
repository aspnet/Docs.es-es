...
En la aplicación de ejemplo, el tamaño del archivo está limitado a 2 MB (se indica en bytes). El límite se proporciona a través de la [configuración](xref:fundamentals/configuration/index) del *appsettings.json* archivo:
...
El límite de solicitud predeterminado ( `maxAllowedContentLength` ) es de 30 millones bytes, que es aproximadamente de 28,6 MB. Personalice el límite en el `web.config` archivo. En el ejemplo siguiente, el límite se establece en 50 MB (52.428.800 bytes):
...
La `maxAllowedContentLength` configuración solo se aplica a IIS. Para obtener más información, consulte [límites `<requestLimits>` de solicitudes ](/iis/configuration/system.webServer/security/requestFiltering/requestLimits/).
...
Para más información, consulte la sección sobre [IIS](#iis).
...
Aumente el tamaño máximo del cuerpo de solicitud para la solicitud HTTP estableciendo <xref:Microsoft.AspNetCore.Builder.IISServerOptions.MaxRequestBodySize%2A?displayProperty=nameWithType> en `Startup.ConfigureServices` . En el ejemplo siguiente, el límite se establece en 50 MB (52.428.800 bytes):
...
Para obtener más información, vea <xref:host-and-deploy/iis/index#iis-options>.
