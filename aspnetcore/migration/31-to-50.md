...
1. En `wwwroot/index.html`:

   * Agregue un `<link>` elemento al `<head>` elemento para los estilos de ámbito.
   * Cambie el elemento que carga el `App` componente.
...
1. En `Shared/MainLayout.razor` , rodea todo el contenido de un `<div>` elemento con un `id` de `page` :
...
1. El archivo base más reciente `wwwroot/css/app.css` incluye los siguientes estilos. Quite los estilos adicionales que dejen los siguientes estilos y los que haya agregado a la aplicación.

    `wwwroot/css/app.css` (los estilos base no incluyen los estilos agregados por el desarrollador):
...
* En la hoja Azure Portal la **autenticación** del registro de aplicaciones, establezca la configuración de la plataforma en **una aplicación de una sola página** con el URI de redireccionamiento de la aplicación.

* También en la hoja **autenticación** , deshabilite la **concesión implícita** de **tokens de acceso** y **tokens de identificador**.
...
### <a name="standalone-no-locblazor-app-with-azure-active-directory-aad-b2c"></a>BlazorAplicación independiente con Azure Active Directory (AAD) B2C
...
### <a name="hosted-no-locblazor-webassembly-app-with-azure-active-directory-aad-or-b2c"></a>Aplicación hospedada Blazor WebAssembly con Azure Active Directory (AAD) o B2C
...
1. En el Azure Portal el registro de la aplicación de la *`Client`* aplicación, quite la configuración de plataforma **Web** .
...
> info: error de autorización de Microsoft. AspNetCore. Authorization. DefaultAuthorizationService [2]. No se cumplen estos requisitos: DenyAnonymousAuthorizationRequirement: requiere un usuario autenticado.
...
1. En el Azure Portal, acceda al [manifiesto de la aplicación](/azure/active-directory/develop/reference-app-manifest).
1. Establezca el [`allowPublicClient`](/azure/active-directory/develop/reference-app-manifest#allowpublicclient-attribute) atributo en `null` o `true` .
