...
<span data-ttu-id="206dd-105">Blazor es un marco nuevo en ASP.NET Core para la creación de interfaces de usuario web interactivas del lado cliente con .NET:</span><span class="sxs-lookup"><span data-stu-id="206dd-105">Blazor is a new framework in ASP.NET Core for building interactive client-side web UI with .NET:</span></span>
...
<span data-ttu-id="206dd-109">Escenarios compatibles con el marco Blazor:</span><span class="sxs-lookup"><span data-stu-id="206dd-109">Blazor framework supported scenarios:</span></span>

* <span data-ttu-id="206dd-110">Componentes de interfaz de usuario reutilizables (componentes de Razor)</span><span class="sxs-lookup"><span data-stu-id="206dd-110">Reusable UI components (Razor components)</span></span>
...
* <span data-ttu-id="206dd-115">Compilación de bibliotecas de componentes con bibliotecas de clases de Razor</span><span class="sxs-lookup"><span data-stu-id="206dd-115">Build component libraries with Razor class libraries</span></span>
...
<span data-ttu-id="206dd-118">Blazor separa la lógica de representación de componentes del modo en el que se aplican las actualizaciones de la interfaz de usuario.</span><span class="sxs-lookup"><span data-stu-id="206dd-118">Blazor decouples component rendering logic from how UI updates are applied.</span></span> <span data-ttu-id="206dd-119">Blazor Server admite el hospedaje de componentes de Razor en el servidor de una aplicación de ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="206dd-119">Blazor Server provides support for hosting Razor components on the server in an ASP.NET Core app.</span></span> <span data-ttu-id="206dd-120">Las actualizaciones de la interfaz de usuario se administran mediante una conexión de SignalR.</span><span class="sxs-lookup"><span data-stu-id="206dd-120">UI updates are handled over a SignalR connection.</span></span> <span data-ttu-id="206dd-121">Blazor Server se admite en ASP.NET Core 3.0.</span><span class="sxs-lookup"><span data-stu-id="206dd-121">Blazor Server is supported in ASP.NET Core 3.0.</span></span>

### <a name="no-locblazor-webassembly-preview"></a><span data-ttu-id="206dd-122">Blazor WebAssembly (versión preliminar)</span><span class="sxs-lookup"><span data-stu-id="206dd-122">Blazor WebAssembly (Preview)</span></span>

<span data-ttu-id="206dd-123">Las aplicaciones Blazor también se pueden ejecutar directamente en el explorador mediante un entorno de ejecución .NET basado en WebAssembly.</span><span class="sxs-lookup"><span data-stu-id="206dd-123">Blazor apps can also be run directly in the browser using a WebAssembly-based .NET runtime.</span></span> <span data-ttu-id="206dd-124">Blazor WebAssembly se encuentra en versión preliminar y *no* se admite en ASP.NET Core 3.0.</span><span class="sxs-lookup"><span data-stu-id="206dd-124">Blazor WebAssembly is in preview and *not* supported in ASP.NET Core 3.0.</span></span> <span data-ttu-id="206dd-125">Blazor WebAssembly se admitirá en una versión futura de ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="206dd-125">Blazor WebAssembly will be supported in a future release of ASP.NET Core.</span></span>

### <a name="no-locrazor-components"></a><span data-ttu-id="206dd-126">Componentes de Razor</span><span class="sxs-lookup"><span data-stu-id="206dd-126">Razor components</span></span>

<span data-ttu-id="206dd-127">Las aplicaciones Blazor se crean a partir de componentes.</span><span class="sxs-lookup"><span data-stu-id="206dd-127">Blazor apps are built from components.</span></span> <span data-ttu-id="206dd-128">Los componentes son fragmentos independientes de la interfaz de usuario, como una página, un cuadro de diálogo o un formulario.</span><span class="sxs-lookup"><span data-stu-id="206dd-128">Components are self-contained chunks of user interface (UI), such as a page, dialog, or form.</span></span> <span data-ttu-id="206dd-129">Los componentes son clases .NET normales que definen la lógica de representación de la interfaz de usuario y los controladores de eventos del lado cliente.</span><span class="sxs-lookup"><span data-stu-id="206dd-129">Components are normal .NET classes that define UI rendering logic and client-side event handlers.</span></span> <span data-ttu-id="206dd-130">Puede crear aplicaciones web interactivas enriquecidas sin JavaScript.</span><span class="sxs-lookup"><span data-stu-id="206dd-130">You can create rich interactive web apps without JavaScript.</span></span>

<span data-ttu-id="206dd-131">Los componentes de Blazor normalmente se crean mediante la sintaxis de Razor, una mezcla natural de HTML y C#.</span><span class="sxs-lookup"><span data-stu-id="206dd-131">Components in Blazor are typically authored using Razor syntax, a natural blend of HTML and C#.</span></span> <span data-ttu-id="206dd-132">Los componentes de Razor se asemejan a las vistas de Razor Pages y MVC en que ambos usan Razor.</span><span class="sxs-lookup"><span data-stu-id="206dd-132">Razor components are similar to Razor Pages and MVC views in that they both use Razor.</span></span> <span data-ttu-id="206dd-133">A diferencia de las páginas y las vistas, que se basan en un modelo de solicitud y respuesta, los componentes se usan específicamente para controlar la composición de la interfaz de usuario.</span><span class="sxs-lookup"><span data-stu-id="206dd-133">Unlike pages and views, which are based on a request-response model, components are used specifically for handling UI composition.</span></span>
...
<span data-ttu-id="206dd-165">Para proporcionar comentarios sobre la interfaz de usuario cuando la conexión se interrumpe, la API de cliente de SignalR se ha ampliado para incluir los siguientes controladores de eventos:</span><span class="sxs-lookup"><span data-stu-id="206dd-165">To provide UI feedback when the connection is interrupted, the SignalR client API has been expanded to include the following event handlers:</span></span>
...
<span data-ttu-id="206dd-192">Los métodos Hub de SignalR usan el [enrutamiento de punto de conexión](xref:fundamentals/routing).</span><span class="sxs-lookup"><span data-stu-id="206dd-192">SignalR hubs use [Endpoint Routing](xref:fundamentals/routing).</span></span> <span data-ttu-id="206dd-193">La conexión de los métodos Hub de SignalR se hizo previamente de manera explícita:</span><span class="sxs-lookup"><span data-stu-id="206dd-193">SignalR hub connection was previously done explicitly:</span></span>
...
<span data-ttu-id="206dd-194">En la versión anterior, los desarrolladores debían conectar controladores, páginas de Razor y centros de conectividad en distintos lugares.</span><span class="sxs-lookup"><span data-stu-id="206dd-194">In the previous version, developers needed to wire up controllers, Razor pages, and hubs in a variety of places.</span></span> <span data-ttu-id="206dd-195">La conexión explícita da lugar a una serie de segmentos de enrutamiento casi idénticos:</span><span class="sxs-lookup"><span data-stu-id="206dd-195">Explicit connection results in a series of nearly-identical routing segments:</span></span>
...
<span data-ttu-id="206dd-196">Los métodos Hub de SignalR 3.0 se pueden enrutar a través del enrutamiento de punto de conexión.</span><span class="sxs-lookup"><span data-stu-id="206dd-196">SignalR 3.0 hubs can be routed via endpoint routing.</span></span> <span data-ttu-id="206dd-197">Con el enrutamiento de punto de conexión, normalmente todo el enrutamiento se puede configurar en `UseRouting`:</span><span class="sxs-lookup"><span data-stu-id="206dd-197">With endpoint routing, typically all routing can be configured in `UseRouting`:</span></span>
...
<span data-ttu-id="206dd-198">Se agregó ASP.NET Core 3.0 SignalR:</span><span class="sxs-lookup"><span data-stu-id="206dd-198">ASP.NET Core 3.0 SignalR added:</span></span>
...
<span data-ttu-id="206dd-204">Las aplicaciones cliente de JavaScript usan SignalR `Subject` (o [RxJS Subject](https://rxjs.dev/api/index/class/Subject)) como argumento `stream` del método Hub `UploadStream` anterior.</span><span class="sxs-lookup"><span data-stu-id="206dd-204">JavaScript client apps use the SignalR `Subject` (or an [RxJS Subject](https://rxjs.dev/api/index/class/Subject)) for the `stream` argument of the `UploadStream` Hub method above.</span></span>
...
## <a name="new-no-locrazor-directives"></a><span data-ttu-id="206dd-213">Nuevas directivas de Razor</span><span class="sxs-lookup"><span data-stu-id="206dd-213">New Razor directives</span></span>

<span data-ttu-id="206dd-214">La lista siguiente contiene nuevas directivas de Razor:</span><span class="sxs-lookup"><span data-stu-id="206dd-214">The following list contains new Razor directives:</span></span>
...
## <a name="no-locidentityserver4-supports-authentication-and-authorization-for-web-apis-and-spas"></a><span data-ttu-id="206dd-219">IdentityServer4 admite la autenticación y la autorización de SPA y API web</span><span class="sxs-lookup"><span data-stu-id="206dd-219">IdentityServer4 supports authentication and authorization for web APIs and SPAs</span></span>

<span data-ttu-id="206dd-220">ASP.NET Core 3.0 ofrece autenticación en aplicaciones de página única (SPA) mediante la compatibilidad con la autorización de API web.</span><span class="sxs-lookup"><span data-stu-id="206dd-220">ASP.NET Core 3.0 offers authentication in Single Page Apps (SPAs) using the support for web API authorization.</span></span> <span data-ttu-id="206dd-221">ASP.NET Core Identity para autenticar y almacenar usuarios se combina con [IdentityServer4](https://identityserver.io/) para implementar OpenID Connect.</span><span class="sxs-lookup"><span data-stu-id="206dd-221">ASP.NET Core Identity for authenticating and storing users is combined with [IdentityServer4](https://identityserver.io/) for implementing OpenID Connect.</span></span>

<span data-ttu-id="206dd-222">IdentityServer4 es un marco de trabajo de OpenID Connect y OAuth 2.0 para ASP.NET Core 3.0.</span><span class="sxs-lookup"><span data-stu-id="206dd-222">IdentityServer4 is an OpenID Connect and OAuth 2.0 framework for ASP.NET Core 3.0.</span></span> <span data-ttu-id="206dd-223">Habilita las características de seguridad siguientes:</span><span class="sxs-lookup"><span data-stu-id="206dd-223">It enables the following security features:</span></span>
...
<span data-ttu-id="206dd-228">Para más información, vea la [documentación de IdentityServer4](http://docs.identityserver.io/en/latest/index.html) o [Autenticación y autorización para SPA](xref:security/authentication/identity/spa).</span><span class="sxs-lookup"><span data-stu-id="206dd-228">For more information, see [the IdentityServer4 documentation](http://docs.identityserver.io/en/latest/index.html) or [Authentication and authorization for SPAs](xref:security/authentication/identity/spa).</span></span>
...
<span data-ttu-id="206dd-253">Se ha eliminado lo siguiente de las plantillas de interfaz de usuario web (Razor Pages, MVC con controlador y vistas):</span><span class="sxs-lookup"><span data-stu-id="206dd-253">The web UI templates (Razor Pages, MVC with controller and views) have the following removed:</span></span>

* <span data-ttu-id="206dd-254">La interfaz de usuario de consentimiento de cookies ya no está incluida.</span><span class="sxs-lookup"><span data-stu-id="206dd-254">The cookie consent UI is no longer included.</span></span> <span data-ttu-id="206dd-255">Para habilitar la función de consentimiento de cookies en una aplicación ASP.NET Core 3.0 generada por plantillas, vea <xref:security/gdpr>.</span><span class="sxs-lookup"><span data-stu-id="206dd-255">To enable the cookie consent feature in an ASP.NET Core 3.0 template-generated app, see <xref:security/gdpr>.</span></span>
