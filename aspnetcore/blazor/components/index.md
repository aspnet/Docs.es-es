...
[Vea o descargue el código de ejemplo](https://github.com/dotnet/AspNetCore.Docs/tree/main/aspnetcore/blazor/common/samples/) ([cómo descargarlo](xref:index#how-to-download-a-sample))
...
> [!IMPORTANT]
> Las directivas [`@using`][2] del archivo `_Imports.razor` solo se aplican a los archivos Razor (`.razor`), por lo que no se aplican a los archivos C# (`.cs`).
...
Debido a la forma en que Blazor representa el contenido secundario, la representación de componentes dentro de un bucle `for` requiere una variable de índice local si se usa la variable de bucle incremental en el contenido del componente secundario:
>
> ```razor
> @for (int c = 0; c < 10; c++)
> {
...
Registre `NotifierService`:

* En Blazor WebAssembly, registre el servicio como singleton en `Program.Main`:

  ```csharp
  builder.Services.AddSingleton<NotifierService>();
  ```

* En Blazor Server, registre el servicio como con ámbito en `Startup.ConfigureServices`:
