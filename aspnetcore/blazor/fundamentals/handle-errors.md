---
title: 'Control de errores en aplicaciones :::no-loc(Blazor)::: de ASP.NET Core'
author: guardrex
description: 'Descubra cómo :::no-loc(Blazor)::: de ASP.NET Core administra las excepciones no controladas y cómo desarrollar aplicaciones que detecten y controlen los errores.'
...
---
# <a name="handle-errors-in-aspnet-core-no-locblazor-apps"></a><span data-ttu-id="3a5a1-103">Control de errores en aplicaciones :::no-loc(Blazor)::: de ASP.NET Core</span><span class="sxs-lookup"><span data-stu-id="3a5a1-103">Handle errors in ASP.NET Core :::no-loc(Blazor)::: apps</span></span>
...
<span data-ttu-id="3a5a1-105">En este artículo se describe cómo :::no-loc(Blazor)::: administra las excepciones no controladas y cómo desarrollar aplicaciones que detecten y controlen los errores.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-105">This article describes how :::no-loc(Blazor)::: manages unhandled exceptions and how to develop apps that detect and handle errors.</span></span>
...
<span data-ttu-id="3a5a1-107">Cuando una aplicación :::no-loc(Blazor)::: no funciona correctamente durante el desarrollo, recibir información detallada del error de la aplicación ayuda a solucionar el problema.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-107">When a :::no-loc(Blazor)::: app isn't functioning properly during development, receiving detailed error information from the app assists in troubleshooting and fixing the issue.</span></span> <span data-ttu-id="3a5a1-108">Cuando se produce un error, en las aplicaciones :::no-loc(Blazor)::: se muestra una barra dorada en la parte inferior de la pantalla:</span><span class="sxs-lookup"><span data-stu-id="3a5a1-108">When an error occurs, :::no-loc(Blazor)::: apps display a gold bar at the bottom of the screen:</span></span>
...
<span data-ttu-id="3a5a1-111">La interfaz de usuario para esta experiencia de control de errores forma parte de las plantillas de proyecto de :::no-loc(Blazor):::.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-111">The UI for this error handling experience is part of the :::no-loc(Blazor)::: project templates.</span></span>

<span data-ttu-id="3a5a1-112">En una aplicación :::no-loc(Blazor WebAssembly):::, personalice la experiencia en el archivo `wwwroot/index.html`:</span><span class="sxs-lookup"><span data-stu-id="3a5a1-112">In a :::no-loc(Blazor WebAssembly)::: app, customize the experience in the `wwwroot/index.html` file:</span></span>
...
<span data-ttu-id="3a5a1-113">En una aplicación :::no-loc(Blazor Server):::, personalice la experiencia en el archivo `Pages/_Host.cshtml`:</span><span class="sxs-lookup"><span data-stu-id="3a5a1-113">In a :::no-loc(Blazor Server)::: app, customize the experience in the `Pages/_Host.cshtml` file:</span></span>
...
<span data-ttu-id="3a5a1-114">Los estilos incluidos en las plantillas de :::no-loc(Blazor)::: (`wwwroot/css/app.css` o `wwwroot/css/site.css`) ocultan el elemento `blazor-error-ui`, que se muestra posteriormente cuando se produce un error:</span><span class="sxs-lookup"><span data-stu-id="3a5a1-114">The `blazor-error-ui` element is hidden by the styles included in the :::no-loc(Blazor)::: templates (`wwwroot/css/app.css` or `wwwroot/css/site.css`) and then shown when an error occurs:</span></span>
...
* [<span data-ttu-id="3a5a1-159">Nueva representación de :::no-loc(Blazor Server):::</span><span class="sxs-lookup"><span data-stu-id="3a5a1-159">:::no-loc(Blazor Server)::: rerendering</span></span>](#blazor-server-prerendering)
...
<span data-ttu-id="3a5a1-162">Cuando :::no-loc(Blazor)::: crea una instancia de un componente:</span><span class="sxs-lookup"><span data-stu-id="3a5a1-162">When :::no-loc(Blazor)::: creates an instance of a component:</span></span>
...
<span data-ttu-id="3a5a1-169">Durante la vigencia de un componente, :::no-loc(Blazor)::: invoca los [métodos de ciclo de vida](xref:blazor/components/lifecycle) siguientes:</span><span class="sxs-lookup"><span data-stu-id="3a5a1-169">During the lifetime of a component, :::no-loc(Blazor)::: invokes the following [lifecycle methods](xref:blazor/components/lifecycle):</span></span>
...
<span data-ttu-id="3a5a1-170">Si cualquier método de ciclo de vida inicia una excepción, de forma sincrónica o asincrónica, la excepción es grave para un circuito de :::no-loc(Blazor Server):::.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-170">If any lifecycle method throws an exception, synchronously or asynchronously, the exception is fatal to a :::no-loc(Blazor Server)::: circuit.</span></span> <span data-ttu-id="3a5a1-171">Para que los componentes traten los errores de los métodos de ciclo de vida, agregue lógica de control de errores.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-171">For components to deal with errors in lifecycle methods, add error handling logic.</span></span>
...
* <span data-ttu-id="3a5a1-220">La excepción no se trata como grave para un circuito de :::no-loc(Blazor Server):::.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-220">The exception isn't treated as fatal to a :::no-loc(Blazor Server)::: circuit.</span></span>
...
### <a name="no-locblazor-server-prerendering"></a><span data-ttu-id="3a5a1-224">Representación previa de :::no-loc(Blazor Server):::</span><span class="sxs-lookup"><span data-stu-id="3a5a1-224">:::no-loc(Blazor Server)::: prerendering</span></span>

<span data-ttu-id="3a5a1-225">Se puede realizar la representación previa de los componentes de :::no-loc(Blazor)::: mediante el [asistente de etiquetas de componente](xref:mvc/views/tag-helpers/builtin-th/component-tag-helper) para que su marcado HTML representado se devuelva como parte de la solicitud HTTP inicial del usuario.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-225">:::no-loc(Blazor)::: components can be prerendered using the [Component Tag Helper](xref:mvc/views/tag-helpers/builtin-th/component-tag-helper) so that their rendered HTML markup is returned as part of the user's initial HTTP request.</span></span> <span data-ttu-id="3a5a1-226">El funcionamiento es el siguiente:</span><span class="sxs-lookup"><span data-stu-id="3a5a1-226">This works by:</span></span>
...
<span data-ttu-id="3a5a1-253">En estos escenarios, se produce un error en un circuito de :::no-loc(Blazor Server)::: afectado y el subproceso normalmente intenta lo siguiente:</span><span class="sxs-lookup"><span data-stu-id="3a5a1-253">In these scenarios, an affected :::no-loc(Blazor Server)::: circuit fails, and the thread usually attempts to:</span></span>
...
<span data-ttu-id="3a5a1-259">La mayoría de los componentes :::no-loc(Blazor)::: se implementan como archivos `.razor` y se compilan para generar lógica que opere en un objeto <xref:Microsoft.AspNetCore.Components.Rendering.RenderTreeBuilder> y representar su salida.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-259">Most :::no-loc(Blazor)::: components are implemented as `.razor` files and are compiled to produce logic that operates on a <xref:Microsoft.AspNetCore.Components.Rendering.RenderTreeBuilder> to render their output.</span></span> <span data-ttu-id="3a5a1-260">Un desarrollador puede implementar de forma manual lógica de <xref:Microsoft.AspNetCore.Components.Rendering.RenderTreeBuilder> mediante código de C# por procedimientos.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-260">A developer may manually implement <xref:Microsoft.AspNetCore.Components.Rendering.RenderTreeBuilder> logic using procedural C# code.</span></span> <span data-ttu-id="3a5a1-261">Para obtener más información, vea <xref:blazor/advanced-scenarios#manual-rendertreebuilder-logic>.</span><span class="sxs-lookup"><span data-stu-id="3a5a1-261">For more information, see <xref:blazor/advanced-scenarios#manual-rendertreebuilder-logic>.</span></span>
