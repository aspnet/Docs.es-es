...
---
# <a name="secure-an-aspnet-core-no-locblazor-webassembly-hosted-app-with-azure-active-directory-b2c"></a><span data-ttu-id="02ea0-103">Protección de una aplicación hospedada :::no-loc(Blazor WebAssembly)::: de ASP.NET Core con Azure Active Directory B2C</span><span class="sxs-lookup"><span data-stu-id="02ea0-103">Secure an ASP.NET Core :::no-loc(Blazor WebAssembly)::: hosted app with Azure Active Directory B2C</span></span>
...
1. <span data-ttu-id="02ea0-114">Indique un **nombre** para la aplicación (por ejemplo, **:::no-loc(Blazor Server)::: AAD B2C** ).</span><span class="sxs-lookup"><span data-stu-id="02ea0-114">Provide a **Name** for the app (for example, **:::no-loc(Blazor Server)::: AAD B2C** ).</span></span>
...
1. <span data-ttu-id="02ea0-136">Indique un **Nombre** para la aplicación (por ejemplo, **:::no-loc(Blazor)::: Client AAD B2C** ).</span><span class="sxs-lookup"><span data-stu-id="02ea0-136">Provide a **Name** for the app (for example, **:::no-loc(Blazor)::: Client AAD B2C** ).</span></span>
...
1. <span data-ttu-id="02ea0-153">Indique un **Nombre** para la aplicación (por ejemplo, **:::no-loc(Blazor)::: Client AAD B2C** ).</span><span class="sxs-lookup"><span data-stu-id="02ea0-153">Provide a **Name** for the app (for example, **:::no-loc(Blazor)::: Client AAD B2C** ).</span></span>
...
1. <span data-ttu-id="02ea0-171">Seleccione la *aplicación de API de servidor* en la columna **Nombre** (por ejemplo, **:::no-loc(Blazor Server)::: AAD B2C** ).</span><span class="sxs-lookup"><span data-stu-id="02ea0-171">Select the *Server API app* from the **Name** column (for example, **:::no-loc(Blazor Server)::: AAD B2C** ).</span></span>
...
### <a name="userno-locidentityname"></a><span data-ttu-id="02ea0-210">User.:::no-loc(Identity):::.Name</span><span class="sxs-lookup"><span data-stu-id="02ea0-210">User.:::no-loc(Identity):::.Name</span></span>

<span data-ttu-id="02ea0-211">`User.:::no-loc(Identity):::.Name` no se rellena de forma predeterminada.</span><span class="sxs-lookup"><span data-stu-id="02ea0-211">By default, the `User.:::no-loc(Identity):::.Name` isn't populated.</span></span>

<span data-ttu-id="02ea0-212">Para configurar la aplicación con el fin de recibir el valor del tipo de notificación `name`, establezca <xref:Microsoft.:::no-loc(Identity):::Model.Tokens.TokenValidationParameters.NameClaimType?displayProperty=nameWithType> de <xref:Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerOptions> en `Startup.ConfigureServices`:</span><span class="sxs-lookup"><span data-stu-id="02ea0-212">To configure the app to receive the value from the `name` claim type, configure the <xref:Microsoft.:::no-loc(Identity):::Model.Tokens.TokenValidationParameters.NameClaimType?displayProperty=nameWithType> of the <xref:Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerOptions> in `Startup.ConfigureServices`:</span></span>
