...
---
# <a name="secure-an-aspnet-core-no-locblazor-webassembly-hosted-app-with-azure-active-directory"></a><span data-ttu-id="f3dc7-103">Protección de una aplicación hospedada :::no-loc(Blazor WebAssembly)::: de ASP.NET Core con Azure Active Directory</span><span class="sxs-lookup"><span data-stu-id="f3dc7-103">Secure an ASP.NET Core :::no-loc(Blazor WebAssembly)::: hosted app with Azure Active Directory</span></span>
...
1. <span data-ttu-id="f3dc7-116">Indique un **nombre** para la aplicación (por ejemplo, **:::no-loc(Blazor Server)::: AAD** ).</span><span class="sxs-lookup"><span data-stu-id="f3dc7-116">Provide a **Name** for the app (for example, **:::no-loc(Blazor Server)::: AAD** ).</span></span>
...
1. <span data-ttu-id="f3dc7-141">Indique un **Nombre** para la aplicación (por ejemplo, **:::no-loc(Blazor)::: Client AAD** ).</span><span class="sxs-lookup"><span data-stu-id="f3dc7-141">Provide a **Name** for the app (for example, **:::no-loc(Blazor)::: Client AAD** ).</span></span>
...
1. <span data-ttu-id="f3dc7-159">Indique un **Nombre** para la aplicación (por ejemplo, **:::no-loc(Blazor)::: Client AAD** ).</span><span class="sxs-lookup"><span data-stu-id="f3dc7-159">Provide a **Name** for the app (for example, **:::no-loc(Blazor)::: Client AAD** ).</span></span>
...
1. <span data-ttu-id="f3dc7-179">Seleccione la *aplicación de API de servidor* en la columna **Nombre** (por ejemplo, **:::no-loc(Blazor Server)::: AAD** ).</span><span class="sxs-lookup"><span data-stu-id="f3dc7-179">Select the *Server API app* from the **Name** column (for example, **:::no-loc(Blazor Server)::: AAD** ).</span></span>
...
<span data-ttu-id="f3dc7-205">La compatibilidad con la autenticación y autorización de llamadas realizadas a las API web de ASP.NET Core con Microsoft :::no-loc(Identity)::: Platform se proporciona mediante los paquetes siguientes:</span><span class="sxs-lookup"><span data-stu-id="f3dc7-205">The support for authenticating and authorizing calls to ASP.NET Core Web APIs with the Microsoft :::no-loc(Identity)::: Platform is provided by the following packages:</span></span>
...
### <a name="userno-locidentityname"></a><span data-ttu-id="f3dc7-218">User.:::no-loc(Identity):::.Name</span><span class="sxs-lookup"><span data-stu-id="f3dc7-218">User.:::no-loc(Identity):::.Name</span></span>
...
<span data-ttu-id="f3dc7-220">Para configurar la aplicación con el fin de recibir el valor del tipo de notificación `name`, establezca <xref:Microsoft.:::no-loc(Identity):::Model.Tokens.TokenValidationParameters.NameClaimType?displayProperty=nameWithType> de <xref:Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerOptions> en `Startup.ConfigureServices`:</span><span class="sxs-lookup"><span data-stu-id="f3dc7-220">To configure the app to receive the value from the `name` claim type, configure the <xref:Microsoft.:::no-loc(Identity):::Model.Tokens.TokenValidationParameters.NameClaimType?displayProperty=nameWithType> of the <xref:Microsoft.AspNetCore.Authentication.JwtBearer.JwtBearerOptions> in `Startup.ConfigureServices`:</span></span>
