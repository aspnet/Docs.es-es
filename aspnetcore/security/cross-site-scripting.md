...
>
> Use uno de los métodos siguientes para evitar que el código se exponga a XSS: _ `createElement()` y asigne valores de propiedad con métodos o propiedades adecuados como `node.textContent=` o `node.InnerText=` .
