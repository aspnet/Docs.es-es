---
title: 'ASP.NET Core 2,1 MVC SameSite :::no-loc(cookie)::: ejemplo'
author: rick-anderson
description: 'ASP.NET Core 2,1 MVC SameSite :::no-loc(cookie)::: ejemplo'
...
<span data-ttu-id="d5fde-108">A continuación se describe un ejemplo de cómo escribir un atributo SameSite en un :::no-loc(cookie)::: :</span><span class="sxs-lookup"><span data-stu-id="d5fde-108">Following is an example of how to write a SameSite attribute on a :::no-loc(cookie)::::</span></span>
...
## <a name="setting-no-loccookie-authentication-and-session-state-no-loccookies"></a><span data-ttu-id="d5fde-109">Establecimiento :::no-loc(Cookie)::: de la autenticación y el estado de la sesión :::no-loc(cookie):::</span><span class="sxs-lookup"><span data-stu-id="d5fde-109">Setting :::no-loc(Cookie)::: Authentication and Session State :::no-loc(cookie):::s</span></span>

<span data-ttu-id="d5fde-110">:::no-loc(Cookie)::: autenticación, estado de sesión y [otros componentes](../samesite.md?view=aspnetcore-2.1) establecen sus opciones de sameSite a través :::no-loc(Cookie)::: de opciones, por ejemplo</span><span class="sxs-lookup"><span data-stu-id="d5fde-110">:::no-loc(Cookie)::: authentication, session state and [various other components](../samesite.md?view=aspnetcore-2.1) set their sameSite options via :::no-loc(Cookie)::: options, for example</span></span>
...
<span data-ttu-id="d5fde-111">En el código anterior, tanto la :::no-loc(cookie)::: autenticación como el estado de la sesión establecen el atributo sameSite en `None` , emitiendo el atributo con un `None` valor y establecen también el atributo Secure en true.</span><span class="sxs-lookup"><span data-stu-id="d5fde-111">In the preceding code, both :::no-loc(cookie)::: authentication and session state set their sameSite attribute to `None`, emitting the attribute with a `None` value, and also set the Secure attribute to true.</span></span>
...
<span data-ttu-id="d5fde-113">Si ejecuta el [proyecto de ejemplo](https://github.com/blowdart/AspNetSameSiteSamples/tree/master/AspNetCore21MVC), cargue el depurador del explorador en la página inicial y úselo para ver la :::no-loc(cookie)::: recopilación del sitio.</span><span class="sxs-lookup"><span data-stu-id="d5fde-113">If you run the [sample project](https://github.com/blowdart/AspNetSameSiteSamples/tree/master/AspNetCore21MVC), load your browser debugger on the initial page and use it to view the :::no-loc(cookie)::: collection for the site.</span></span> <span data-ttu-id="d5fde-114">Para ello, en Edge y Chrome, presione la `F12` `Application` tecla TAB y haga clic en la dirección URL del sitio en la `:::no-loc(Cookie):::s` opción de la `Storage` sección.</span><span class="sxs-lookup"><span data-stu-id="d5fde-114">To do so in Edge and Chrome press `F12` then select the `Application` tab and click the site URL under the `:::no-loc(Cookie):::s` option in the `Storage` section.</span></span>

![Depurador del explorador::: no-LOC (cookie)::: List](BrowserDebugger.png)

<span data-ttu-id="d5fde-116">Puede ver en la imagen anterior que el :::no-loc(cookie)::: creado por el ejemplo al hacer clic en el botón "crear SameSite :::no-loc(Cookie)::: " tiene un valor de atributo SameSite de `Lax` , que coincide con el valor establecido en el [código de ejemplo](#sampleCode).</span><span class="sxs-lookup"><span data-stu-id="d5fde-116">You can see from the image above that the :::no-loc(cookie)::: created by the sample when you click the "Create SameSite :::no-loc(Cookie):::" button has a SameSite attribute value of `Lax`, matching the value set in the [sample code](#sampleCode).</span></span>

## <a name="intercepting-no-loccookies"></a><a name="interception"></a><span data-ttu-id="d5fde-117">Interceptar :::no-loc(cookie):::</span><span class="sxs-lookup"><span data-stu-id="d5fde-117">Intercepting :::no-loc(cookie):::s</span></span>

<span data-ttu-id="d5fde-118">Para interceptar :::no-loc(cookie)::: s, para ajustar el valor NONE según su compatibilidad en el agente del explorador del usuario, debe usar el `:::no-loc(Cookie):::Policy` middleware.</span><span class="sxs-lookup"><span data-stu-id="d5fde-118">In order to intercept :::no-loc(cookie):::s, to adjust the none value according to its support in the user's browser agent you must use the `:::no-loc(Cookie):::Policy` middleware.</span></span> <span data-ttu-id="d5fde-119">Debe colocarse en la canalización de solicitudes HTTP **antes** que los componentes que escriben :::no-loc(cookie)::: y se configuran en `ConfigureServices()` .</span><span class="sxs-lookup"><span data-stu-id="d5fde-119">This must be placed into the http request pipeline **before** any components that write :::no-loc(cookie):::s and configured within `ConfigureServices()`.</span></span>

<span data-ttu-id="d5fde-120">Para insertarlo en el uso de la canalización `app.Use:::no-loc(Cookie):::Policy()` en el `Configure(IApplicationBuilder, IHostingEnvironment)` método de [Startup.CS](https://github.com/blowdart/AspNetSameSiteSamples/blob/master/AspNetCore21MVC/Startup.cs).</span><span class="sxs-lookup"><span data-stu-id="d5fde-120">To insert it into the pipeline use `app.Use:::no-loc(Cookie):::Policy()` in the `Configure(IApplicationBuilder, IHostingEnvironment)` method in [Startup.cs](https://github.com/blowdart/AspNetSameSiteSamples/blob/master/AspNetCore21MVC/Startup.cs).</span></span> <span data-ttu-id="d5fde-121">Por ejemplo:</span><span class="sxs-lookup"><span data-stu-id="d5fde-121">For example:</span></span>
...
<span data-ttu-id="d5fde-122">Después, en la `ConfigureServices(IServiceCollection services)` configuración de la :::no-loc(cookie)::: Directiva que se va a llamar a una clase auxiliar cuando :::no-loc(cookie)::: se anexan o eliminan.</span><span class="sxs-lookup"><span data-stu-id="d5fde-122">Then in the `ConfigureServices(IServiceCollection services)` configure the :::no-loc(cookie)::: policy to call out to a helper class when :::no-loc(cookie):::s are appended or deleted.</span></span> <span data-ttu-id="d5fde-123">Por ejemplo:</span><span class="sxs-lookup"><span data-stu-id="d5fde-123">For example:</span></span>
...
<span data-ttu-id="d5fde-124">La función auxiliar `CheckSameSite(HttpContext, :::no-loc(Cookie):::Options)` :</span><span class="sxs-lookup"><span data-stu-id="d5fde-124">The helper function `CheckSameSite(HttpContext, :::no-loc(Cookie):::Options)`:</span></span>

* <span data-ttu-id="d5fde-125">Se llama a cuando se :::no-loc(cookie)::: anexan a la solicitud o se eliminan de la solicitud.</span><span class="sxs-lookup"><span data-stu-id="d5fde-125">Is called when :::no-loc(cookie):::s are appended to the request or deleted from the request.</span></span>
