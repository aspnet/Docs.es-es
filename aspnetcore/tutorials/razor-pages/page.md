---
title: 'Parte 3. :::no-loc(Razor)::: Pages con scaffolding en ASP.NET Core'
author: rick-anderson
description: 'Parte 3 de la serie de tutoriales sobre :::no-loc(Razor)::: Pages.'
ms.author: riande
ms.date: 08/17/2019
no-loc:
...
---
# <a name="part-3-scaffolded-no-locrazor-pages-in-aspnet-core"></a><span data-ttu-id="d500c-103">Parte 3. :::no-loc(Razor)::: Pages con scaffolding en ASP.NET Core</span><span class="sxs-lookup"><span data-stu-id="d500c-103">Part 3, scaffolded :::no-loc(Razor)::: Pages in ASP.NET Core</span></span>
...
<span data-ttu-id="d500c-108">Las instancias de :::no-loc(Razor)::: Pages derivan de `PageModel`.</span><span class="sxs-lookup"><span data-stu-id="d500c-108">:::no-loc(Razor)::: Pages are derived from `PageModel`.</span></span> <span data-ttu-id="d500c-109">Por convención, la clase derivada de `PageModel` se denomina `<PageName>Model`.</span><span class="sxs-lookup"><span data-stu-id="d500c-109">By convention, the `PageModel`-derived class is called `<PageName>Model`.</span></span> <span data-ttu-id="d500c-110">El constructor aplica la [inserción de dependencias](xref:fundamentals/dependency-injection) para agregar el `:::no-loc(Razor):::PagesMovieContext` a la página.</span><span class="sxs-lookup"><span data-stu-id="d500c-110">The constructor uses [dependency injection](xref:fundamentals/dependency-injection) to add the `:::no-loc(Razor):::PagesMovieContext` to the page.</span></span> <span data-ttu-id="d500c-111">Todas las páginas con scaffolding siguen este patrón.</span><span class="sxs-lookup"><span data-stu-id="d500c-111">All the scaffolded pages follow this pattern.</span></span> <span data-ttu-id="d500c-112">Vea [Código asincrónico](xref:data/ef-rp/intro#asynchronous-code) para obtener más información sobre programación asincrónica con Entity Framework.</span><span class="sxs-lookup"><span data-stu-id="d500c-112">See [Asynchronous code](xref:data/ef-rp/intro#asynchronous-code) for more information on asynchronous programming with Entity Framework.</span></span>

<span data-ttu-id="d500c-113">Cuando se efectúa una solicitud de la página, el método `OnGetAsync` devuelve una lista de películas a la instancia de :::no-loc(Razor)::: Pages.</span><span class="sxs-lookup"><span data-stu-id="d500c-113">When a request is made for the page, the `OnGetAsync` method returns a list of movies to the :::no-loc(Razor)::: Page.</span></span> <span data-ttu-id="d500c-114">Se llama a `OnGetAsync` o `OnGet` para inicializar el estado de la página.</span><span class="sxs-lookup"><span data-stu-id="d500c-114">`OnGetAsync` or `OnGet` is called to initialize the state of the page.</span></span> <span data-ttu-id="d500c-115">En este caso, `OnGetAsync` obtiene una lista de películas y las muestra.</span><span class="sxs-lookup"><span data-stu-id="d500c-115">In this case, `OnGetAsync` gets a list of movies and displays them.</span></span>
...
<a name="index"></a> <span data-ttu-id="d500c-119">Examine la instancia de :::no-loc(Razor)::: Pages *Pages/Movies/Index.cshtml* :</span><span class="sxs-lookup"><span data-stu-id="d500c-119">Examine the *Pages/Movies/Index.cshtml* :::no-loc(Razor)::: Page:</span></span>

[!code-cshtml[](razor-pages-start/snapshot_sample3/:::no-loc(Razor):::PagesMovie30/Pages/Movies/Index.cshtml)]

<span data-ttu-id="d500c-120">:::no-loc(Razor)::: puede realizar la transición de HTML a C# o a un marcado específico de :::no-loc(Razor):::.</span><span class="sxs-lookup"><span data-stu-id="d500c-120">:::no-loc(Razor)::: can transition from HTML into C# or into :::no-loc(Razor):::-specific markup.</span></span> <span data-ttu-id="d500c-121">Cuando el símbolo `@` va seguido de una [palabra clave reservada de :::no-loc(Razor):::](xref:mvc/views/razor#razor-reserved-keywords), se realiza una transición a un marcado específico de :::no-loc(Razor):::; en caso contrario, la transición se realiza a C#.</span><span class="sxs-lookup"><span data-stu-id="d500c-121">When an `@` symbol is followed by a [:::no-loc(Razor)::: reserved keyword](xref:mvc/views/razor#razor-reserved-keywords), it transitions into :::no-loc(Razor):::-specific markup, otherwise it transitions into C#.</span></span>
...
<span data-ttu-id="d500c-123">La directiva `@page` de :::no-loc(Razor)::: convierte el archivo en una acción de MVC, lo que significa que puede administrar las solicitudes.</span><span class="sxs-lookup"><span data-stu-id="d500c-123">The `@page` :::no-loc(Razor)::: directive makes the file an MVC action, which means that it can handle requests.</span></span> <span data-ttu-id="d500c-124">`@page` debe ser la primera directiva de :::no-loc(Razor)::: de una página.</span><span class="sxs-lookup"><span data-stu-id="d500c-124">`@page` must be the first :::no-loc(Razor)::: directive on a page.</span></span> <span data-ttu-id="d500c-125">`@page` es un ejemplo de la transición a un marcado específico de :::no-loc(Razor):::.</span><span class="sxs-lookup"><span data-stu-id="d500c-125">`@page` is an example of transitioning into :::no-loc(Razor):::-specific markup.</span></span> <span data-ttu-id="d500c-126">Vea [Sintaxis de :::no-loc(Razor):::](xref:mvc/views/razor#razor-syntax) para obtener más información.</span><span class="sxs-lookup"><span data-stu-id="d500c-126">See [:::no-loc(Razor)::: syntax](xref:mvc/views/razor#razor-syntax) for more information.</span></span>
...
<span data-ttu-id="d500c-133">La directiva `@model` especifica el tipo del modelo que se pasa a una instancia de :::no-loc(Razor)::: Pages.</span><span class="sxs-lookup"><span data-stu-id="d500c-133">The `@model` directive specifies the type of the model passed to the :::no-loc(Razor)::: Page.</span></span> <span data-ttu-id="d500c-134">En el ejemplo anterior, la línea `@model` permite que la clase derivada de `PageModel` esté disponible en la instancia de :::no-loc(Razor)::: Pages.</span><span class="sxs-lookup"><span data-stu-id="d500c-134">In the preceding example, the `@model` line makes the `PageModel`-derived class available to the :::no-loc(Razor)::: Page.</span></span> <span data-ttu-id="d500c-135">El modelo se usa en los [asistentes de HTML](/aspnet/mvc/overview/older-versions-1/views/creating-custom-html-helpers-cs#understanding-html-helpers) `@Html.DisplayNameFor` y `@Html.DisplayFor` de la página.</span><span class="sxs-lookup"><span data-stu-id="d500c-135">The model is used in the `@Html.DisplayNameFor` and `@Html.DisplayFor` [HTML Helpers](/aspnet/mvc/overview/older-versions-1/views/creating-custom-html-helpers-cs#understanding-html-helpers) on the page.</span></span>
...
<span data-ttu-id="d500c-137">Seleccione los vínculos de menú ( **:::no-loc(Razor):::PagesMovie** [Película de Razor Pages], **Home** [Inicio] y **Privacy** [Privacidad]).</span><span class="sxs-lookup"><span data-stu-id="d500c-137">Select the menu links ( **:::no-loc(Razor):::PagesMovie** , **Home** , and **Privacy** ).</span></span> <span data-ttu-id="d500c-138">Cada página muestra el mismo diseño de menú.</span><span class="sxs-lookup"><span data-stu-id="d500c-138">Each page shows the same menu layout.</span></span> <span data-ttu-id="d500c-139">El diseño de menú se implementa en el archivo *Pages/Shared/_Layout.cshtml*.</span><span class="sxs-lookup"><span data-stu-id="d500c-139">The menu layout is implemented in the *Pages/Shared/_Layout.cshtml* file.</span></span> <span data-ttu-id="d500c-140">Abra el archivo *Pages/Shared/_Layout.cshtml*.</span><span class="sxs-lookup"><span data-stu-id="d500c-140">Open the *Pages/Shared/_Layout.cshtml* file.</span></span>
...
<span data-ttu-id="d500c-156">La línea `@*Markup removed for brevity.*@` es un comentario de :::no-loc(Razor):::.</span><span class="sxs-lookup"><span data-stu-id="d500c-156">The line `@*Markup removed for brevity.*@` is a :::no-loc(Razor)::: comment.</span></span> <span data-ttu-id="d500c-157">A diferencia de los comentarios HTML (`<!-- -->`), los comentarios de :::no-loc(Razor)::: no se envían al cliente.</span><span class="sxs-lookup"><span data-stu-id="d500c-157">Unlike HTML comments (`<!-- -->`), :::no-loc(Razor)::: comments are not sent to the client.</span></span>
...
<span data-ttu-id="d500c-159">Cambie el elemento `<title>` del archivo *Pages/Shared/_Layout.cshtml* para mostrar **Movie** en lugar de **:::no-loc(Razor):::PagesMovie**.</span><span class="sxs-lookup"><span data-stu-id="d500c-159">Change the `<title>` element in the *Pages/Shared/_Layout.cshtml* file to display **Movie** rather than **:::no-loc(Razor):::PagesMovie**.</span></span>
...
<span data-ttu-id="d500c-175">El marcado anterior establece el archivo de diseño de todos los archivos de :::no-loc(Razor)::: de la carpeta *Pages* en *Pages/Shared/_Layout.cshtml*.</span><span class="sxs-lookup"><span data-stu-id="d500c-175">The preceding markup sets the layout file to *Pages/Shared/_Layout.cshtml* for all :::no-loc(Razor)::: files under the *Pages* folder.</span></span> <span data-ttu-id="d500c-176">Vea [Layout](xref:razor-pages/index#layout) (Diseño) para más información.</span><span class="sxs-lookup"><span data-stu-id="d500c-176">See [Layout](xref:razor-pages/index#layout) for more information.</span></span>
...
### <a name="the-create-no-locrazor-page"></a><span data-ttu-id="d500c-191">La página de creación de instancias de :::no-loc(Razor)::: Pages</span><span class="sxs-lookup"><span data-stu-id="d500c-191">The Create :::no-loc(Razor)::: Page</span></span>

<span data-ttu-id="d500c-192">Examine el archivo de instancia de :::no-loc(Razor)::: Pages *Pages/Movies/Create.cshtml* :</span><span class="sxs-lookup"><span data-stu-id="d500c-192">Examine the *Pages/Movies/Create.cshtml* :::no-loc(Razor)::: Page file:</span></span>
...
<span data-ttu-id="d500c-202">El motor de scaffolding crea un marcado de :::no-loc(Razor)::: para cada campo del modelo (excepto el identificador) similar al siguiente:</span><span class="sxs-lookup"><span data-stu-id="d500c-202">The scaffolding engine creates :::no-loc(Razor)::: markup for each field in the model (except the ID) similar to the following:</span></span>
...
<span data-ttu-id="d500c-211">En este tutorial se examinan las instancias de :::no-loc(Razor)::: Pages creadas con la técnica scaffolding en el [tutorial anterior](xref:tutorials/razor-pages/model).</span><span class="sxs-lookup"><span data-stu-id="d500c-211">This tutorial examines the :::no-loc(Razor)::: Pages created by scaffolding in the [previous tutorial](xref:tutorials/razor-pages/model).</span></span>

<span data-ttu-id="d500c-212">[Vea o descargue](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/tutorials/razor-pages/razor-pages-start/sample/:::no-loc(Razor):::PagesMovie22) un ejemplo.</span><span class="sxs-lookup"><span data-stu-id="d500c-212">[View or download](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/tutorials/razor-pages/razor-pages-start/sample/:::no-loc(Razor):::PagesMovie22) sample.</span></span>
...
<span data-ttu-id="d500c-215">Las instancias de :::no-loc(Razor)::: Pages derivan de `PageModel`.</span><span class="sxs-lookup"><span data-stu-id="d500c-215">:::no-loc(Razor)::: Pages are derived from `PageModel`.</span></span> <span data-ttu-id="d500c-216">Por convención, la clase derivada de `PageModel` se denomina `<PageName>Model`.</span><span class="sxs-lookup"><span data-stu-id="d500c-216">By convention, the `PageModel`-derived class is called `<PageName>Model`.</span></span> <span data-ttu-id="d500c-217">El constructor aplica la [inserción de dependencias](xref:fundamentals/dependency-injection) para agregar el `:::no-loc(Razor):::PagesMovieContext` a la página.</span><span class="sxs-lookup"><span data-stu-id="d500c-217">The constructor uses [dependency injection](xref:fundamentals/dependency-injection) to add the `:::no-loc(Razor):::PagesMovieContext` to the page.</span></span> <span data-ttu-id="d500c-218">Todas las páginas con scaffolding siguen este patrón.</span><span class="sxs-lookup"><span data-stu-id="d500c-218">All the scaffolded pages follow this pattern.</span></span> <span data-ttu-id="d500c-219">Vea [Código asincrónico](xref:data/ef-rp/intro#asynchronous-code) para obtener más información sobre programación asincrónica con Entity Framework.</span><span class="sxs-lookup"><span data-stu-id="d500c-219">See [Asynchronous code](xref:data/ef-rp/intro#asynchronous-code) for more information on asynchronous programming with Entity Framework.</span></span>

<span data-ttu-id="d500c-220">Cuando se efectúa una solicitud de la página, el método `OnGetAsync` devuelve una lista de películas a la instancia de :::no-loc(Razor)::: Pages.</span><span class="sxs-lookup"><span data-stu-id="d500c-220">When a request is made for the page, the `OnGetAsync` method returns a list of movies to the :::no-loc(Razor)::: Page.</span></span> <span data-ttu-id="d500c-221">`OnGetAsync` o `OnGet` se invocan en una instancia de :::no-loc(Razor)::: Pages para inicializar el estado de esa instancia.</span><span class="sxs-lookup"><span data-stu-id="d500c-221">`OnGetAsync` or `OnGet` is called on a :::no-loc(Razor)::: Page to initialize the state for the page.</span></span> <span data-ttu-id="d500c-222">En este caso, `OnGetAsync` obtiene una lista de películas y las muestra.</span><span class="sxs-lookup"><span data-stu-id="d500c-222">In this case, `OnGetAsync` gets a list of movies and displays them.</span></span>
...
<a name="index"></a> <span data-ttu-id="d500c-226">Examine la instancia de :::no-loc(Razor)::: Pages *Pages/Movies/Index.cshtml* :</span><span class="sxs-lookup"><span data-stu-id="d500c-226">Examine the *Pages/Movies/Index.cshtml* :::no-loc(Razor)::: Page:</span></span>

[!code-cshtml[](razor-pages-start/snapshot_sample/:::no-loc(Razor):::PagesMovie/Pages/Movies/Index.cshtml)]

<span data-ttu-id="d500c-227">:::no-loc(Razor)::: puede realizar la transición de HTML a C# o a un marcado específico de :::no-loc(Razor):::.</span><span class="sxs-lookup"><span data-stu-id="d500c-227">:::no-loc(Razor)::: can transition from HTML into C# or into :::no-loc(Razor):::-specific markup.</span></span> <span data-ttu-id="d500c-228">Cuando el símbolo `@` va seguido de una [palabra clave reservada de :::no-loc(Razor):::](xref:mvc/views/razor#razor-reserved-keywords), se realiza una transición a un marcado específico de :::no-loc(Razor):::; en caso contrario, la transición se realiza a C#.</span><span class="sxs-lookup"><span data-stu-id="d500c-228">When an `@` symbol is followed by a [:::no-loc(Razor)::: reserved keyword](xref:mvc/views/razor#razor-reserved-keywords), it transitions into :::no-loc(Razor):::-specific markup, otherwise it transitions into C#.</span></span>

<span data-ttu-id="d500c-229">La directiva de `@page` de :::no-loc(Razor)::: convierte el archivo en una acción de MVC, lo que significa que puede controlar las solicitudes.</span><span class="sxs-lookup"><span data-stu-id="d500c-229">The `@page` :::no-loc(Razor)::: directive makes the file into an MVC action, which means that it can handle requests.</span></span> <span data-ttu-id="d500c-230">`@page` debe ser la primera directiva de :::no-loc(Razor)::: de una página.</span><span class="sxs-lookup"><span data-stu-id="d500c-230">`@page` must be the first :::no-loc(Razor)::: directive on a page.</span></span> <span data-ttu-id="d500c-231">`@page` es un ejemplo de la transición a un marcado específico de :::no-loc(Razor):::.</span><span class="sxs-lookup"><span data-stu-id="d500c-231">`@page` is an example of transitioning into :::no-loc(Razor):::-specific markup.</span></span> <span data-ttu-id="d500c-232">Vea [Sintaxis de :::no-loc(Razor):::](xref:mvc/views/razor#razor-syntax) para obtener más información.</span><span class="sxs-lookup"><span data-stu-id="d500c-232">See [:::no-loc(Razor)::: syntax](xref:mvc/views/razor#razor-syntax) for more information.</span></span>
...
<span data-ttu-id="d500c-239">La directiva `@model` especifica el tipo del modelo que se pasa a una instancia de :::no-loc(Razor)::: Pages.</span><span class="sxs-lookup"><span data-stu-id="d500c-239">The `@model` directive specifies the type of the model passed to the :::no-loc(Razor)::: Page.</span></span> <span data-ttu-id="d500c-240">En el ejemplo anterior, la línea `@model` permite que la clase derivada de `PageModel` esté disponible en la instancia de :::no-loc(Razor)::: Pages.</span><span class="sxs-lookup"><span data-stu-id="d500c-240">In the preceding example, the `@model` line makes the `PageModel`-derived class available to the :::no-loc(Razor)::: Page.</span></span> <span data-ttu-id="d500c-241">El modelo se usa en los [asistentes de HTML](/aspnet/mvc/overview/older-versions-1/views/creating-custom-html-helpers-cs#understanding-html-helpers) `@Html.DisplayNameFor` y `@Html.DisplayFor` de la página.</span><span class="sxs-lookup"><span data-stu-id="d500c-241">The model is used in the `@Html.DisplayNameFor` and `@Html.DisplayFor` [HTML Helpers](/aspnet/mvc/overview/older-versions-1/views/creating-custom-html-helpers-cs#understanding-html-helpers) on the page.</span></span>
...
<span data-ttu-id="d500c-260">La línea `@*Markup removed for brevity.*@` es un comentario de :::no-loc(Razor)::: que no aparece en el archivo de diseño.</span><span class="sxs-lookup"><span data-stu-id="d500c-260">The line `@*Markup removed for brevity.*@` is a :::no-loc(Razor)::: comment which doesn't appear in your layout file.</span></span> <span data-ttu-id="d500c-261">A diferencia de los comentarios HTML (`<!-- -->`), los comentarios de :::no-loc(Razor)::: no se envían al cliente.</span><span class="sxs-lookup"><span data-stu-id="d500c-261">Unlike HTML comments (`<!-- -->`), :::no-loc(Razor)::: comments are not sent to the client.</span></span>
...
<span data-ttu-id="d500c-263">Cambie el elemento `<title>` del archivo *Pages/Shared/_Layout.cshtml* para mostrar **Movie** en lugar de **:::no-loc(Razor):::PagesMovie**.</span><span class="sxs-lookup"><span data-stu-id="d500c-263">Change the `<title>` element in the *Pages/Shared/_Layout.cshtml* file to display **Movie** rather than **:::no-loc(Razor):::PagesMovie**.</span></span>
...
<span data-ttu-id="d500c-279">El marcado anterior establece el archivo de diseño de todos los archivos de :::no-loc(Razor)::: de la carpeta *Pages* en *Pages/Shared/_Layout.cshtml*.</span><span class="sxs-lookup"><span data-stu-id="d500c-279">The preceding markup sets the layout file to *Pages/Shared/_Layout.cshtml* for all :::no-loc(Razor)::: files under the *Pages* folder.</span></span> <span data-ttu-id="d500c-280">Vea [Layout](xref:razor-pages/index#layout) (Diseño) para más información.</span><span class="sxs-lookup"><span data-stu-id="d500c-280">See [Layout](xref:razor-pages/index#layout) for more information.</span></span>
...
### <a name="the-create-no-locrazor-page"></a><span data-ttu-id="d500c-295">La página de creación de instancias de :::no-loc(Razor)::: Pages</span><span class="sxs-lookup"><span data-stu-id="d500c-295">The Create :::no-loc(Razor)::: Page</span></span>

<span data-ttu-id="d500c-296">Examine el archivo de instancia de :::no-loc(Razor)::: Pages *Pages/Movies/Create.cshtml* :</span><span class="sxs-lookup"><span data-stu-id="d500c-296">Examine the *Pages/Movies/Create.cshtml* :::no-loc(Razor)::: Page file:</span></span>
...
<span data-ttu-id="d500c-306">El motor de scaffolding crea un marcado de :::no-loc(Razor)::: para cada campo del modelo (excepto el identificador) similar al siguiente:</span><span class="sxs-lookup"><span data-stu-id="d500c-306">The scaffolding engine creates :::no-loc(Razor)::: markup for each field in the model (except the ID) similar to the following:</span></span>
